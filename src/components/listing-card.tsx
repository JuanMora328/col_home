...
export function ListingCard({ listing }: { listing: ListingPreview }) {
  return <article className="overflow-hidden rounded-card bg-white shadow-soft">
    ...
    <div className="p-6"><span className="chip">{types[listing.property_type]}</span><h3 className="mt-4 font-heading text-xl font-semibold">{listing.neighborhood}</h3><p className="mt-1 text-ink-muted">{listing.city_name}</p><div className="mt-4 flex gap-2 text-sm"><span className="chip-neutral">{listing.bedrooms} hab.</span><span className="chip-neutral">{listing.bathrooms} baños</span></div><div className="mt-5 border-t border-surface-high pt-4">{listing.monthly_price === 0 ? <><p className="text-sm font-semibold text-secondary">Alojamiento gratuito</p><p className="text-xl font-bold">$0</p></> : <p className="text-xl font-bold">{money.format(listing.monthly_price)} <span className="text-base font-normal">/ mes</span></p>}</div></div>
  </article>;
}
