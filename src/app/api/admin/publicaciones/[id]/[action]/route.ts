...
const operations = {
  ...
  string,
  { allowed: readonly ListingStatus[]; next: ListingStatus; message: string }
>;
...
export async function POST(
  ...
  if (
    !current ||
    !(operation.allowed as readonly ListingStatus[]).includes(current)
  )
    return redirect(`/admin/publicaciones/${id}?message=invalid`);

  const { error } = await admin
    .from("listings")
    .update({ status: operation.next })
    .eq("id", id)
    .eq("status", current);
