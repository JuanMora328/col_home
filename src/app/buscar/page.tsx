...
const integer = (value: string, maximum: number) => { const parsed = /^\d+$/.test(value) ? Number(value) : NaN; return Number.isSafeInteger(parsed) && parsed <= maximum ? parsed : undefined; };

export const metadata: Metadata = {
  title: "Buscar viviendas | Colombia Abriga",
  description: "Encuentra viviendas en arriendo o alojamientos temporales gratuitos disponibles en Colombia.",
};
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const filters = { department: getDepartments().some((d) => d.code === department) ? department : "", city, maxPrice: integer(one(params.maxPrice), 2_000_000_000), propertyType, minBedrooms: integer(one(params.minBedrooms), 100), availabilityType, page };
